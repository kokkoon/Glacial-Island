...
const caseTwo = (string) => {
    try {
        ...
        var hotFormulaParser = new FormulaParser();
        const data = hotFormulaParser.parse(string);
        if (!data.error) {
            string = data.result
        } else {
            throw false
